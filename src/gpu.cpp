...
uint64_t get_shader_source_hash(int shader_type_index)
{
    ...
    const char* name = layer_shader_registry[shader_type_index].name;
    uint64_t h = 0xcbf29ce484222325ull;
    h = fnv1a_64_update(h, (const unsigned char*)name, (int)strlen(name));
    return h;
}
...
PipelineCache* VulkanDevice::get_pipeline_cache() const
{
...
int compile_spirv_module(const char*, const Option&, std::vector<uint32_t>&)
{
    NCNN_LOGE("runtime shader compilation was removed; preload the fixed-model spir-v cache");
    return -1;
}

int compile_spirv_module(const char*, int, const Option&, std::vector<uint32_t>&)
{
    NCNN_LOGE("runtime shader compilation was removed; preload the fixed-model spir-v cache");
    return -1;
}
...
int compile_spirv_module(int shader_type_index, const Option& opt, std::vector<uint32_t>& spirv)
{
    ...
    (void)opt;
    (void)spirv;
    NCNN_LOGE("no precompiled spir-v module for shader %s", layer_shader_registry[shader_type_index].name);
    return -1;
}
